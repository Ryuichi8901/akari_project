...
def main() -> None:
    ...
    try:
        ...
        log = {"good_count": 0, "peace_count": 0}  # peace_countを初期化
    total_good_count = log.get("good_count", 0)
    total_peace_count = log.get("peace_count", 0)  # パーのカウントを初期化

    renderer = HandFaceRenderer(tracker=tracker, output=None)
    HAND_GOOD_THRESHOLD = 5  # グーのジェスチャーがこの回数以上検出されたら1いいねとカウント
    HAND_PEACE_THRESHOLD = 5  # パーのジェスチャーがこの回数以上検出されたら1パーとカウント
    HAND_ERROR_THRESHOLD = 3  # グー以外がこの回数以上検出されたらリセット
    GOOD_LOCK_TIME = 2  # いいね後のロック時間
    last_count_time = time.time()
    hand_good_count = [0, 0]  # グーのカウント回数[左手、右手]
    hand_good_error_count = [0, 0]  # グー以外が抜けた回数[左手、右手]
    hand_peace_count = [0, 0]  # パーのカウント回数[左手、右手]
    hand_peace_error_count = [0, 0]  # パー以外が抜けた回数[左手、右手]
    display_good_count(total_good_count)
    ...
    while True:
        ...
        for hand in hands:
            ...
            # パーの処理
            if gesture_result == "PEACE" and time.time() - last_count_time > GOOD_LOCK_TIME:
                hand_peace_count[hand_num] += 1
            ...
            # パーの検出回数が一定回数になったらパーを加算
            if hand_peace_count[hand_num] > HAND_PEACE_THRESHOLD:
                last_count_time = time.time()
                total_peace_count += 1
                hand_peace_count[hand_num] = 0
                hand_peace_error_count[hand_num] = 0
                
                # パーのカウントをJSONに更新
                result = {"good_count": total_good_count, "peace_count": total_peace_count}
                with open(json_path, mode="wt", encoding="utf-8") as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
            ...
            # エラーカウント処理
                ...
                hand_good_count[hand_num] = 0
                hand_good_error_count[hand_num] = 0
            elif hand_peace_error_count[hand_num] > HAND_ERROR_THRESHOLD:
                hand_peace_count[hand_num] = 0
                hand_peace_error_count[hand_num] = 0
